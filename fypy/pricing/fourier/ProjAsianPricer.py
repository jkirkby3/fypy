...
class ProjAsianPricer(ProjPricer):
    ...
    def price_strikes(
        self, T: float = None, M: int = None, K: np.ndarray = None, is_calls=None
    ) -> np.ndarray:
        """
    ...
    def price(
        self, T: float = None, M: int = None, K: float = None, is_call: bool = None
    ) -> float:
        """
        override of the method of StrikesPricer in order to handle new parameters
        """
        prices = self.price_strikes(
            T=T, K=np.asarray([K]), is_calls=np.asarray([is_call]), M=M
        )
        return prices[0]
...
class ProjArithmeticAsianPricer(ProjAsianPricer):
    ...
    def __init__(
        ...
        super(ProjArithmeticAsianPricer, self).__init__(
            model=model, N=N, L=L, order=order, alpha_override=alpha_override
        )
    ...
    def price_strikes_fill(
        ...
        option_params = np.array(
            [(self.get_model().spot(), T, M, T / M, self.get_model().spot() / (M + 1))],
            dtype=[
                ("S_0", float),
                ("T", float),
                ("M", int),
        ...
        # setting most of the grid parameters
        self._grid_setting(grid_params=grid_params)

        # impl object creation
        impl = CubicImpl(
            N=self._N,
            dx=grid_params["dx"].item(),
            model=self._model,
            T=option_params["dt"].item(),
            max_n_bar=grid_params["nbar"].item(),
        )

        # establish the initial values that will be uniform across all strikes.
        # this calculation is consolidated into a single method because, depending on nbar,
        # it may need to be repeated if the grid is not sufficiently wide.
        # This method includes:
        # - benhamou shift arrays computation
        # - orthogonal projection coefficients computation
        # - payoff constants for arithmetic asian option
        x1, beta, payoff_coefficient_constants = self._initial_values_setting(
            option_params=option_params, grid_params=grid_params, impl=impl
        )

        for index in range(len(K)):
            # update of the nbar value and related ystar,
            # and assess the necessity for expanding the numerical grid.
            # if the numerical grid need to be expanded, initial values will be updated inside this method
            self._grid_update(
            ...
            beta_adjusted = ProjArithmeticAsianPricer._beta_strike_adjustment(
                option_params=option_params,
                grid_params=grid_params,
                impl=impl,
                x1=x1,
            ...
            G = ProjArithmeticAsianPricer._payoff_coefficient_computation(
                option_params=option_params,
                grid_params=grid_params,
                payoff_coefficient_constants=payoff_coefficient_constants,
                index=index,
    ...
    def _grid_setting(self, grid_params: np.ndarray):
    ...
    def benhamou_shift_computation(
        self, option_params: np.ndarray, grid_params: np.ndarray
    ):

        # ER : risk neutral expected return over increment dt=1/M (can set to zero if it is unknown)
        ER = (
            self.get_r(option_params["T"].item())
            - self.get_q(option_params["T"].item())
            + self.get_model().convexity_correction()
        ) * (option_params["dt"].item())

        x1 = np.zeros(option_params["M"].item())
        x1[0] = ER

        for m in range(1, option_params["M"].item()):
            x1[m] = ER + np.log(1 + np.exp(x1[m - 1]))  # BENHAMOU SHIFT

        Nm = np.floor(grid_params["a"].item() * (x1 - ER))
        x1 = (
            ER
            + (1 - self._N / 2) * grid_params["dx"].item()
            + Nm * grid_params["dx"].item()
        )
        return x1, Nm
    ...
    def _sig_computation():
        ...
        # Computation of sig is required for the _PSI_computation method,
        # which itself is essential for the functioning of the _beta_computation method
    ...
    def _PSI_computation(
        N: int, M: int, dx: float, a: float, x1: np.ndarray, Nm: np.ndarray
    ):
        NNM = int(N + Nm[M - 2])  # Number of columns of PSI
        Neta = 5 * NNM + 15  # Sample size
        Neta5 = NNM + 3

        thet = ProjArithmeticAsianPricer._thet_computation(
            dx=dx, x1=x1, Neta=Neta, Neta5=Neta5
        )
    ...
    def _beta_computation(
        self,
        option_params: np.ndarray = None,
        grid_params: np.ndarray = None,
        impl: Impl = None,
        x1: np.ndarray = None,
        Nm: np.ndarray = None,
    ):
        ...
        beta = np.concatenate(
            (
                [grid_params["AA"].item()],
                impl.zeta() * PhiR[1 : self._N] * np.exp(-1j * x1[0] * xi),
            )
        )
        ...
        PSI = ProjArithmeticAsianPricer._PSI_computation(
            N=self.get_N(),
            M=option_params["M"].item(),
            dx=grid_params["dx"].item(),
            a=grid_params["a"].item(),
            x1=x1,
            Nm=Nm,
        )

        # Convert both matrices to complex128 before using np.dot(),
        # this will be necessary when using a just-in-time compiler
        beta = (
            np.dot(PSI[:, :N].astype(np.complex128), beta.astype(np.complex128)) * PhiR
        )

        for m in range(2, option_params["M"].item()):
            beta[1:N] = impl.zeta() * beta[1:N] * np.exp(-1j * x1[m - 1] * xi)
            beta[0] = grid_params["AA"].item()
            beta = np.real(np.fft.fft(beta))
            beta = (
                np.dot(
                    PSI[:, int(Nm[m - 1]) : int(Nm[m - 1]) + N].astype(np.complex128),
    ...
    def asian_coefficient_constants(C: float, dx: float, impl: Impl):
        ...
        constants = np.asarray(
            [
                (1 / 24 - impl.g1) * np.exp(-dx),
                0.5 - impl.g2,
                (23 / 24 - impl.g3) * np.exp(dx),
                impl.g4 / 90,
            ]
        )
    ...
    def _initial_values_setting(
        self, option_params: np.ndarray, grid_params: np.ndarray, impl: Impl
    ):
        # benhamou shift arrays computation
        x1, Nm = self.benhamou_shift_computation(
            option_params=option_params, grid_params=grid_params
        )

        # orthogonal projection coefficients computation
        beta = self._beta_computation(
            option_params=option_params,
            grid_params=grid_params,
            x1=x1,
            Nm=Nm,
        ...
        payoff_coefficient_constants = (
            ProjArithmeticAsianPricer.asian_coefficient_constants(
                C=option_params["C"].item(), dx=grid_params["dx"].item(), impl=impl
            )
        )

        return x1, beta, payoff_coefficient_constants

    def _initial_values_update(
        self,
        option_params: np.ndarray,
        grid_params: np.ndarray,
        impl: Impl,
        x1: np.ndarray,
        beta: np.ndarray,
        payoff_coefficient_constants: np.ndarray,
    ):
        ...
        # the initial values obtained using _initial_values_setting must be re-computed.

        # to utilize a just-in-time compiler in the near future,
        # we maintain numpy array structures and update values by reference,
        # instead of using dictionaries or objects.
        ...
        # the attributes of the impl object are reassigned
        for attr in vars(new_impl):
            setattr(impl, attr, getattr(new_impl, attr))

        new_initial_values = self._initial_values_setting(
            option_params=option_params, grid_params=grid_params, impl=impl
        )

        # the values of the initial values [x1, beta, payoff_coefficient_constants] are reassigned
        for initial, new in zip(
            [x1, beta, payoff_coefficient_constants], new_initial_values
        ):
            initial[:] = new
    ...
    def _grid_widening(
        ...
        while grid_params["nbar"].item() + 1 > self._N:
            ...
            grid_params["alpha"] = max(
                1.15 * grid_params["alpha"].item(),
                1.15 * np.max(np.abs(np.log(K / self.get_model().spot())))
                + self.get_model().cumulants(option_params["T"].item()).c1,
            )
            ...
            # update of the nbar value and related ystar
            ProjArithmeticAsianPricer._nbar_update(
                option_params=option_params,
                grid_params=grid_params,
                x1=x1,
                K=K,
        ...
        # since the alpha has been changed, all the initial values outside initial the for cycle must be updated
        self._initial_values_update(
            option_params=option_params,
            grid_params=grid_params,
            impl=impl,
            x1=x1,
    ...
    def _nbar_update(
        ...
        # update of the nbar value and related ystar
        grid_params["ystar"] = np.log(
            (option_params["M"].item() + 1) * K[index] / option_params["S_0"].item() - 1
        )
        grid_params["nbar"] = int(
            np.floor(
                (grid_params["ystar"].item() - x1[option_params["M"].item() - 1])
                * grid_params["a"]
                + 1
            )
        )
    ...
    def _grid_update(
        ...
        # assess the necessity for expanding the numerical grid
        if grid_params["nbar"].item() + 1 > self._N:
            self._grid_widening(
                option_params=option_params,
                grid_params=grid_params,
                impl=impl,
    ...
    def _beta_strike_adjustment(
        ...
        x1_adjusted, beta_adjusted = ProjArithmeticAsianPricer.copy_original_arrays(
            x1, beta
        )

        # shift adjustment,
        # the information related to the strike is contained inside ystar and nbar
        x1_adjusted[option_params["M"].item() - 1] = (
            grid_params["ystar"].item()
            - (grid_params["nbar"].item() - 1) * grid_params["dx"].item()
        )
        ...
        beta_adjusted[1:N] = (
            impl.zeta()
            * beta_adjusted[1:N]
            * np.exp(-1j * x1_adjusted[option_params["M"].item() - 1] * impl.w[1:])
        )
    ...
    @staticmethod
    def _payoff_coefficient_computation(
        option_params: np.ndarray,
        grid_params: np.ndarray,
        payoff_coefficient_constants: np.ndarray,
        index: int,
        ...
        G = np.zeros(nbar + 1)
        E = np.exp(
            grid_params["ystar"].item() - (nbar - 1) * dx + dx * np.arange(nbar + 1)
        )
        D = K[index] - option_params["C"].item()

        G[nbar] = D / 24 - payoff_coefficient_constants[0] * E[nbar]
        G[nbar - 1] = 0.5 * D - payoff_coefficient_constants[1] * E[nbar - 1]
    ...
    def _price_update(
        ...
        Val = (
            grid_params["C_aN"].item()
            * np.exp(-r * T)
            * np.sum(beta_adjusted[0 : grid_params["nbar"].item() + 1] * G)
        )
        ...
        if is_calls[index]:  # Call Option, use Put-Call-Parity
            ...
            Val = (
                Val
                + option_params["C"].item() * np.exp(-r * T) * mult
                - K[index] * np.exp(-r * T)
            )
